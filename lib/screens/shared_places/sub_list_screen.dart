              ...
              builder: (context, snapshot) {
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (_, index) => _PlaceCard(
                    item: snapshot.data![index] as Map<String, dynamic>,
...
class _PlaceCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final description =
        item['desc'] as String? ??
        item['introduction'] as String? ??
        item['location']?['address'] as String? ??
        'ไม่มีรายละเอียด';
    final thumbnailList = item['thumbnailUrl'];
    final thumbnailUrl = (thumbnailList is List && thumbnailList.isNotEmpty)
        ? thumbnailList[0] as String
        : '';
      ...
      onTap: () {
        if (collectionName == 'otopProducts') {
          ...
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (_) => AttractionDetailScreen(placeData: item),
            ),
          );
        }
      },
  ...
  Widget _buildPlaceholder() {
    return Container(
      ...
      child: Icon(
        collectionName == 'otopProducts' ? Symbols.shopping_bag : Symbols.image,
        color: Colors.grey,
      ),
    );
  }
