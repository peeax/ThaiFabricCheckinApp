...
const sortedEntries = [...provinceEntries].sort(
...
export function removeProvince(text: string, englishProvince: string): string {
  ...
  if (trimmed.toLowerCase() === englishProvince.toLowerCase()) {
    return "";
  }

  const trailingProvince = new RegExp(
    `,\\s*${escapeRegExp(englishProvince)}\\s*$`,
    "i",
  );
